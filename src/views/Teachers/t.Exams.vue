<template>
  <v-container fluid class="rounded-lg bg-surface">
    <v-card flat>
      <v-card-title class="d-flex align-center pe-2 bg-secondary">
        <v-icon icon="mdi-pencil"></v-icon> &nbsp; Manage Exmas
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      try {
        // Call the API to get exams along with options and responses
        const response = await axios.get(
          "fetch-exams-with-options-and-responses"
        );
        this.exams = response.data;
      } catch (error) {
        console.error("Failed to fetch exams and related data:", error);
      }
    },
    getColor(status) {
